...
export class GroqProvider implements ILlmProvider {
  ...
  async chat(
    ...
    try {
      ...
      if (!response.ok) {
        throw new LlmProviderError('Groq request failed.', 'provider_error', true);
      }
      ...
      const normalizedCalls = normalizeToolCalls('groq', rawToolCalls);
      ...
      if ((error as Error).name === 'AbortError') {
        throw new LlmProviderError('Groq timeout.', 'timeout', true);
      }
      throw new LlmProviderError('Groq provider error.', 'provider_error', true);
    } finally {
      clearTimeout(timer);
    }
  }
